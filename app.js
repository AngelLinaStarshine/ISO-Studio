...
/* ---------- Adaptive engine ---------- */
const Eng = {
  prof:{ level:1, cx:1, xp:0, streak:0, best:0, hist:[], mastery:{}, seedN:1 },
  async load(){
    const p=await Store.get('iso_profile_v1');
    if(p) this.prof=Object.assign(this.prof,p);
  },
  ...
  record(score, passed, usedHint){
    ...
    if(passed){
      ...
      // ascend when the learner is clearly improving
      const last3=p.hist.slice(-3);
      const strong = last3.length===3 && last3.every(r=>r.passed && r.score>=0.88 &&
                       r.l===p.level && r.c===p.cx);
      if(strong){
        if(p.cx < MAX_CX[p.level]) { p.cx++; this._evt='Stage up — shapes get more complex.'; }
        else if(p.level<7){ p.level++; p.cx=1; this._evt=`Level up → ${LEVEL_NAMES[p.level]}!`; }
        else this._evt='Mastery — you topped the ladder!';
      } else this._evt=null;
    } else {
      p.streak=0;
      const last2=p.hist.slice(-2);
      const struggling = last2.length===2 && last2.every(r=>!r.passed && r.l===p.level && r.c===p.cx);
      if(struggling){
        if(p.cx>1){ p.cx--; this._evt='Easing back a stage — keep going.'; }
        else if(p.level>1){ p.level--; p.cx=MAX_CX[p.level]; this._evt='Stepping back one level for practice.'; }
        else this._evt=null;
      } else this._evt=null;
    }
    return this._evt;
...
function nearestDot(x,y){
  ...
  return (bd <= (view.S*0.5)**2) ? best : null;
}
...
function loadTask(t){
  ...
  document.getElementById('bCheck').style.display='';
...
function edgeHit(x,y){ // nearest committed edge within tolerance
  ...
  return bd <= (view.S*0.22)**2 ? best : null;
}
  ...
  if(d && !(d.A===anchor.A&&d.B===anchor.B)){
    ...
    if(segs){
      ...
      if(feedback){ feedback=null; document.getElementById('verdict').className='verdict';
        document.getElementById('scoreWrap').style.display='none';
        document.getElementById('bCheck').style.display=''; document.getElementById('bNext').style.display='none'; }
...
document.getElementById('bCheck').onclick=async()=>{
  ...
  feedback={g};
  const evt=Eng.record(g.score,g.passed,usedHint);
  await Eng.save();
  const v=document.getElementById('verdict');
  v.className='verdict show '+(g.passed?'pass':'fail');
  v.innerHTML = (g.passed?'✓ Correct sketch!':'Not quite — keep refining')+
...
document.getElementById('bNext').onclick=()=>newAdaptiveTask();
